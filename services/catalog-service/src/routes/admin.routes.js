...
import {
  listGenres, createGenre, deleteGenre, createGame, updateGame, deleteGame
...
const storage = multer.diskStorage({
  destination: async (req, _file, cb) => {
    ...
    const [[g]] = await pool.query(
      'SELECT id, game_code FROM games WHERE id=? OR game_code=? LIMIT 1',
      [key, key]
    );
...
r.post('/admin/games/:idOrCode/images', upload.array('files', 12), async (req, res) => {
  const key = req.params.idOrCode;
  const [[g]] = await pool.query(
    'SELECT id, game_code, hero_image FROM games WHERE id=? OR game_code=? LIMIT 1',
    [key, key]
  );
  ...
  if (pick) await pool.query('UPDATE games SET hero_image=? WHERE id=?', [pick, g.id]);
...
r.delete('/admin/games/:idOrCode/images/:file', async (req, res) => {
  ...
  await pool.query('DELETE FROM game_images WHERE game_id=? AND image_url=?',
    [g.id, `/uploads/${g.game_code}/${file}`]);
  res.json({ ok: true });
});
