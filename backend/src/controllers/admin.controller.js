...
export async function createGame(req,res){
  let { title, platform, release_date, developer, publisher, trailer_url, description, steam_app_id, genre_ids=[] } = req.body;
  if (!title || !platform || !release_date) return res.status(400).json({error:'title, platform, release_date requeridos'});
  ...
  const [r] = await pool.query(
    `INSERT INTO games (game_code,title,slug,platform,developer,publisher,release_date,trailer_url,description,steam_app_id)
     VALUES (?,?,?,?,?,?,?,?,?,?)`,
...
export async function updateGame(req,res){
  ...
  const [[g]] = await pool.query(`SELECT id FROM games WHERE id=? OR game_code=? LIMIT 1`,[key,key]);
  ...
  await pool.query(
    `UPDATE games SET title=?, platform=?, release_date=?, developer=?, publisher=?, trailer_url=?, description=?, steam_app_id=? WHERE id=?`,
